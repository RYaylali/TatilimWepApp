...
using Tatilim.WebUI.Dtos.WorkLocationDto;
...
namespace Tatilim.WebUI.Controllers
{
	public class WorkLocationController : Controller
	{
		...
		[HttpPost]
		public async Task<IActionResult> UpdateRoom(UpdateWorkLocationDto model)
		{
			var client = _httpClientFactory.CreateClient();
			var jsonData = JsonConvert.SerializeObject(model);
			StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
			var responseMessage = await client.PutAsync("http://localhost:5200/api/WorkLocation", stringContent);
			if (responseMessage.IsSuccessStatusCode)
			{
				return RedirectToAction("Index");
			}
